# 已归档实现计划：F-003 单 NPC 角色化真实对话

状态：`completed / archive_prepared_in_PR_3`。

本计划服务于已归档的 [`F-003` 任务卡](F-003-single-npc-real-dialogue.md)。各 Step 均按前一 Step 证据和用户明确授权执行；未复用 F-001、F-002、外部调用或 Git 授权，也未进入 R-04。
...
| Git 交付 | `archive_prepared_in_PR_3` | 精确提交 56 个 F-003 文件，push 功能分支并创建 PR #3；首轮远程 CI 通过后在同一 PR 准备归档 | 归档 HEAD 必须再次通过 fake-only CI；最终合并事实以 GitHub 为准；不进入 R-04 |
...
- 用户 UAT、真实 provider 评估、独立 QA 和最终本地门禁全部通过，Step 7 当时状态收口为 `ready_for_git_delivery`；8000/8001 均已释放，后续 Git 交付另获用户授权，未进入 R-04。
...
功能提交 `41527ecca161cccb4878ad5289f0b29b163e0f17` 和首轮 GitHub-hosted Linux `Quality` 均由 GitHub PR #3 记录。任务卡与实现计划在同一 PR 准备归档，归档 HEAD 必须重新通过远程 CI；最终合并和 merge SHA 以 GitHub 为准，不复用授权进入 R-04。
