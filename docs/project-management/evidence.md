...
| F-004 当前范围与下一门禁 | 任务卡、阶段地图、roadmap、Step 7 用户 UAT、fake-only 修复及本地门禁 | 当前唯一活动任务为 F-004；Step 0–7 完成，空历史虚构记忆已修复并经用户真实 Godot 窗口复验；完整统一门禁 pytest 584 passed，9 健康 + 10 对话 loopback 全通过 | 当前 `ready_for_git_delivery`；Git/PR/CI/归档及 R-05 均未执行；Step 5 首次失败请求 usage 缺失持续如实披露 |
