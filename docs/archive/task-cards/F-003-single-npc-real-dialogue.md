# 已归档任务：F-003 单 NPC 角色化真实对话

状态：`completed / archive_prepared_in_PR_3`。

来源：[`roadmap.md`](../../project-management/roadmap.md) 的 `R-03`。用户已批准任务卡、Step 5 真实 DeepSeek 验收、Step 6 独立 QA、已发现问题的修复、Step 7 用户 UAT 和 F-003 Git 交付；Step 0–7 均已完成，首轮 3 项 P1、6 项 P2 及复审新增的同类畸形 `choices` P2 均已关闭，两名独立 reviewer 最终均为 NO FINDINGS。用户真实窗口 UAT、本地门禁与首轮 GitHub-hosted Linux CI 均已通过；GitHub PR #3 是统一交付与归档载体，最终 CI 和合并事实以 GitHub 为准。
...
- Step 0 只更新本任务卡和对应的 [`implementation-plan.md`](F-003-implementation-plan.md)。
...
- [x] 功能提交、PR、首轮 Linux CI 与任务归档已准备；最终 CI 和合并事实以 GitHub PR #3 为准；
...
功能提交为 `41527ecca161cccb4878ad5289f0b29b163e0f17`，分支为 `feat/f-003-single-npc-real-dialogue`，统一交付载体为 GitHub PR #3；首个功能 HEAD 的 GitHub-hosted Linux `Quality` 已通过。任务卡和实现计划在同一 PR 准备归档，归档 HEAD 必须再次通过 CI，最终合并状态以 GitHub 为准；未额外调用真实 provider，未进入 R-04。
