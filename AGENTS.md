...
- 未经用户明确授权不得删除临时资源；“不得擅自删除”不免除主动汇报义务。资源到期时必须列出保留/删除建议、影响范围和可恢复性，并请求处置授权。
- Step、QA、UAT 与 Git 交付的完成、阻塞、取消、崩溃恢复和会话移交都必须更新持久化台账并执行收口门禁。用户可见报告须将仍在使用、已到期、用户已删除、由 Codex 获准删除和明确保留的资源分别列示；不得仅在 evidence 或过程日志中记录而省略汇报。
- 申请回收授权前，必须盘点目标内 tracked、untracked、ignored、嵌套仓库和可能敏感的资源类别，只读取处置所需元数据，不读取秘密或数据库内容；授权清单必须明确这些资源会随目标一并删除及其可恢复性。
