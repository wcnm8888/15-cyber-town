# 当前实现计划：F-004 短期会话记忆与上下文预算

状态：`approved / step_7_complete / ready_for_git_delivery`。

本计划只服务于已批准的 [`F-004 当前任务卡`](current-task.md)。Step 0–7 已完成；空历史虚构记忆已完成 fake-only 失败优先修复、准确调用台账补录、用户真实 Godot 窗口零调用复验及完整统一门禁。当前为 `ready_for_git_delivery`，Git 交付须另行授权，不得进入 R-05。
...
| Git 交付 | `not_authorized` | 精确暂存提交、push 功能分支、PR、fake-only CI、合并和归档 | 另获明确 Git 授权；无 secrets、数据库、运行时数据或 R-05 实现 |
