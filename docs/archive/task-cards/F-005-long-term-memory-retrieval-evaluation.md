# 已归档任务：F-005 长期记忆与检索评估

状态：`completed / archive_prepared_in_PR_5`。

来源：[`roadmap.md`](../../project-management/roadmap.md) 的 `R-05`。前置任务 F-001—F-004 均已通过 PR #1—#4 交付并归档；F-005 起始 `main` 基线为 `3e03d64d129871495f3fe73295ee9b11478f2e71`。用户已批准 F-005 Git 交付；Step 0—7、真实模型评估、独立 QA、用户真实 Godot UAT、最终本地门禁及首轮 GitHub Linux CI 均已完成。统一交付与归档载体为 PR #5，最终归档 HEAD 的 CI 和合并事实以 GitHub 为准，不得进入 R-06。
...
- 最终 fake-only 统一入口 `1095 passed`，既定 lock、ruff、mypy、Schema、Godot、loopback、ignore/sensitive 与 diff 门禁全部通过；Step 7 收口时达到 `ready_for_git_delivery`。

## 交付、归档与运行时资源

- 功能提交为 `338852e4dd03f8c679f8a2db920e2ba7bd6f968e`，功能分支为 `feat/f-005-long-term-memory-retrieval-evaluation`，统一交付载体为 GitHub PR #5；首个功能 HEAD 的 GitHub-hosted Linux `quality` 已通过。
- 任务卡与实现计划在同一 PR 准备归档；归档 HEAD 必须再次通过 CI，最终合并与 merge SHA 以 GitHub 为准，不在合并前伪称已合并。
- 已盘点并保留受 Git 忽略的 Step 5/Step 7 隔离验收 SQLite 和 metadata-only 调用台账；不删除、提交或读取正式业务数据库内容，不进入 R-06。
