# 已归档任务：F-004 短期会话记忆与上下文预算

状态：`completed / archive_prepared_in_PR_4`。

来源：[`roadmap.md`](../../project-management/roadmap.md) 的 `R-04`。用户已批准 F-004 Git 交付；Step 0–7、真实模型同 scope 回忆、独立 QA、空历史失败优先修复和用户真实 Godot 窗口零调用复验均已完成。功能提交和首轮 GitHub-hosted Linux CI 已通过；统一交付载体为 GitHub PR #4，最终归档 HEAD 的 CI 和合并事实以 GitHub 为准，不得进入 R-05。
...
- 本文件是 F-004 已归档任务卡；[`F-004-implementation-plan.md`](F-004-implementation-plan.md) 是已归档 Step 地图。
...
- [x] Step 7：单独授权的用户真实窗口 UAT、零调用修复点复验与最终本地交付审查。
- [x] Git 交付准备：另获授权的功能提交、push、PR #4、首轮 Linux CI 与同 PR 任务归档；最终归档 HEAD 的 CI 和合并事实以 GitHub 为准。
...
功能提交为 `eb8a9cc69a93f004a346e8d27f81b10fa3c89a46`，功能分支为 `feat/f-004-short-term-memory-context-budget`，统一交付载体为 GitHub PR #4；首个功能 HEAD 的 GitHub-hosted Linux `quality` 已通过。任务卡与实现计划在同一 PR 准备归档，归档 HEAD 必须再次通过 CI；最终合并与 merge SHA 以 GitHub 为准。F-004 实际真实调用 16 次，已核算 15 次费用 USD 0.00402380，首次 Step 5 失败请求 usage 不可追溯；最终用户复验与 Git 交付均零新增真实模型调用，未进入 R-05。
